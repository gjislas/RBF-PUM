...
#include <stdio.h>
#include <math.h>
#include "NoFLA_HQRRP_WY_blk_var4_single.h"
...
#define THRESHOLD_FOR_SGEQPF   250
#define THRESHOLD_FOR_SGEQP3  1000
...
static float NoFLA_Normal_random_number( float mu, float sigma );
  ...
  if( num_factorized_fixed_cols > 0 ) {
    ...
    if( num_factorized_fixed_cols < n_A ) {
      ...
      sormqr_( "Left", "Transpose", 
               ...
               work, lwork, info );
      if( * info != 0 ) {
        fprintf( stderr, "ERROR in sormqr: Info: %d \n", * info );
      }
...
// ============================================================================
int NoFLA_HQRRP_WY_blk_var4( int m_A, int n_A, float * buff_A, int ldim_A,
...
//
  ...
  if( m_A < 0 ) {
    fprintf( stderr, 
             "ERROR in NoFLA_HQRRP_WY_blk_var4_single: m_A is < 0.\n" );
  } if( n_A < 0 ) {
    fprintf( stderr, 
             "ERROR in NoFLA_HQRRP_WY_blk_var4_single: n_A is < 0.\n" );
  } if( ldim_A < max( 1, m_A ) ) {
    fprintf( stderr, 
             "ERROR in NoFLA_HQRRP_WY_blk_var4_single: ldim_A is < max( 1, m_A ).\n" );
  }
...
static float NoFLA_Normal_random_number( float mu, float sigma ) {
  ...
  factor = sqrt( ( -2 * log( a ) ) / a ); ///////////////// Maybe check this later
  b1 = c1 * factor;
  b2 = c2 * factor;
  alternate_calls = ! alternate_calls;
  return( mu + sigma * b1 );
